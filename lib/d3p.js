...
d3p.animate = {
  object: function(key, object, params){
      var params = params || {};
      if(d3p.transitions[key].setup) d3p.transitions[key].setup(object, params);
      d3p.transitions[key].run(object, params, function(){});
  },
  parallel: function(key, objects, params){
      d3p.runner.add(function(done){
        var i  = 0,
        os     = d3p.helpers.toArray(objects),
        n      = os.length,
        params = params || {},
        check  = function(){
          i++;
        ...
        for(var o = 0; o < n; o++){
          if(d3p.transitions[key].setup) d3p.transitions[key].setup(os[o], params);
          d3p.transitions[key].run(os[o], params, check);
        }
      });
  },
  sequence: function(key, objects, params){
    var i  = 0,
    os     = d3p.helpers.toArray(objects),
    n      = os.length,
    params = params || {};
    ...
    for(var o = 0; o < n; o++){
      (function(object){
        d3p.runner.add(function(done){
          d3p.transitions[key].run(object, params, done);
        });
      })(os[o]);
    }
  }
